...
def main(cfg: DictConfig) -> None:
    ...
    with stage_run(cfg, "monitor") as context_stamp:
        ...
        def score_windows(source: pd.DataFrame, label: str, bug=None, bug_from: int | None = None):
            ...
            for window_id, index in enumerate(simulated_windows(source, cfg)):
                ...
                values["month"] = month
                result = run.add(
                    {**thresholded_only(values), "window_id": window_id, "month": month}
                )
                rows.append(
                    {
                        **values,
        ...
        for spec in cfg.monitor.bugs.specs:
            ...
            quiet_after = next(
                (
                    r
                    for r in quiet_bug_run.results
                    if r.level == ALERT and r.window_id >= bug_window
                ),
                None,
            )
            quiet_delay = (
        ...
        update_section(
            ...
            | {
                "natural_drift": {
                    k: v for k, v in payload["natural_drift"].items() if k != "windows"
                },
                "variants": {
                    k: {n: m for n, m in v.items() if n != "windows"} for k, v in variants.items()
                },
                "bugs": {
                    k: {n: m for n, m in v.items() if n != "windows"} for k, v in bugs.items()
                },
            },
            context_stamp,
        )
        log.info(
            "wrote reports/monitoring.json and metrics.json -> monitoring (%.1fs)",
            payload["seconds"],
        )
