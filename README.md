...
|  |  |
| --- | --- |
| **Detection** | ROC-AUC 0.891 / 0.895 on months 6 and 7, catching 57.2% / 58.4% of fraud at the operating threshold. |
| **The guarantee that broke** | At most 1.0% of genuine applicants were promised extra verification. 1.6% / 1.3% got it. The promise holds only while new applications look like the calibration month, and they stop. |
| **The guarantee that held** | 55% of fraud was promised a non-approval. 58.1% / 59.1% got one -- over-delivered, which is the safe direction and still a finding. |
| **Fairness** | False alarms among genuine applicants are 0.433 / 0.443 as likely for the younger group as the older one. 1.00 would be parity. The model never sees age. |
| **What fairness cost** | Neither model-level mitigation beat simply dropping age. The constrained learner reached 0.192 by catching 3.8% of fraud -- equal treatment by way of flagging almost nobody. |
| **Monitoring, without labels** | 4 of 4 injected data faults were caught, on a stream carrying no fraud labels at all. On 200 clean windows the detectors raised 6 watches and 0 alerts. |
| **Shipped** | 6.4 ms per scored request at the median, against a 15 ms target, in a 767 MB image. |
<!-- /metrics:headline -->

The rest of this section is the evidence for those six lines, in the order it was
produced. The two that matter most are the pair of guarantees: one held and one
did not, and the one that did not is the more interesting result.
...
column does move the FPR ratio in the right direction — the fairness table above
puts the champion, which never sees age, well ahead of B1, which does — but it
lands nowhere near parity, and it does so at almost no cost in detection. The
information was never really in that column. (The two models no longer share
hyper-parameters, so that gap is a comparison of two fitted models, not a
controlled ablation.)
...
hyper-parameter search ran with the hand-set parameters scored alongside it as
trial −1, so it could be seen to win or lose — without that, "best of 30" says
nothing about whether the search was worth running. It won, but only one trial in
thirty beat the starting point, and the margin is smaller than the standard error
of a single validation month at this prevalence. The winner was taken because both
held-out months moved the same way, not because one month settled it. Every trial,
including the incumbent, is in
[`reports/tables/tuning_trials.csv`](reports/tables/tuning_trials.csv), so the
spread can be judged rather than taken on trust — and a reader who looks at it and
concludes "noise" is making a defensible call.
...
rule — at 85%, and prints where they actually stand. CI runs it on every push,
because a coverage target nothing enforces is one nobody has to meet.
...
[docs and datasheet](https://github.com/feedzai/bank-account-fraud)) under its own
terms, which at the time of writing are stated on that page as **CC BY-NC-ND 4.0**
— attribution, non-commercial, no derivatives. Feedzai's supporting code repository
carries a separate Apache 2.0 licence.

**No BAF data is committed here.** Not the raw CSVs, not the interim parquet, not
a sample. CI fails the build if any appears. What *is* committed is aggregate
output computed from it — metric summaries, the alpha sweep, the figures — and
whether those count as a derivative work under that licence is a question for a
reader who needs to be sure, not something this README answers. Anyone wanting the
data should download it from Kaggle themselves; `make data` does exactly that and
